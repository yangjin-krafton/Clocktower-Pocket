 ...
 * @param {{name:string, dates:string[], rangeStart:string, rangeEnd:string}} params
 * @returns {Promise<{success:boolean}>}
 */
export async function registerDates({ name, dates, rangeStart, rangeEnd }) {
  if (!PARTY_API_URL) throw new Error('API URL이 설정되지 않았습니다')
  const res = await fetch(PARTY_API_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'text/plain' },
    body: JSON.stringify({ action: 'register', name, dates, rangeStart, rangeEnd }),
  })
  if (!res.ok) throw new Error(`HTTP ${res.status}`)
  return res.json()
}
