...
export class PlayerApp {
  ...
  _showSeatLayout() {
    ...
    const availW   = this.content.getBoundingClientRect().width || (acRect?.width || 360)
    const availH   = (acRect?.height || 520) - 80 - 26
    const contentH = (acRect?.height || 520) - 80

    // 식탁 배치: 화면 비율로 4변에 인원 분배 (세로 긴 quad)
    const R        = availW / availH
    const nH_total = Math.round(playerCount * R / (R + 1))
    const nV_total = playerCount - nH_total
